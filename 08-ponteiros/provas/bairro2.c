...
void compute_votes(int num_votes, int votes[num_votes], int num_parties,
                   Party parties[num_parties], Party *most_voted,
  ...
  *second_most_voted = parties[0];
  ...
  printf("VENCEDOR: %s (%d votos = %.2f%%)\n", most_voted->deputado,
         most_voted->votos, prct_vencedor);
  printf("VICE: %s (%d votos = %.2f%%)", second_most_voted->deputado,
         second_most_voted->votos, prct_vice);
}
...
int main() {
  ...
  compute_votes(qtd_votos, votacao, qtd_chapas, chapas, &maisVotado,
                &segundoMaisVotado);
